// --- LOGOS & HELPER ---
        ...
        // --- FETCH TOP 1 COLOMBIA TMDB (ESTRENO RECIENTE EXCLUSIVO NETFLIX) ---
        async function fetchTop1Colombia() {
            const container = document.getElementById('hero-dynamic-card');
            if (!container) return;

            try {
                // Calcular fecha dinámica de hace 30 días
                ...
                // Fetch con el filtro de fecha dinámica, proveedor Netflix (8) y filtro Colombia
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    // Generamos la tarjeta con altura completa para que rellene el contenedor ajustado
                    container.innerHTML = `
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            renderNav();
            renderAllCategories();
            fetchTop1Colombia(); // Ejecuta la conexión a TMDB para la tarjeta dinámica
            
            initScrollAnimations(); // Inicia las animaciones de scroll
        ...
        // --- RENDER NAV (FILTROS E-COMMERCE CON NAVEGACIÓN LIBRE) ---
        function renderNav() {
            const navContainer = document.getElementById('category-nav');
            const categories = [
                { id: 'recomendaciones', label: '🍿 Recomendaciones', external: true },
            ...
            // Se duplica el contenido para generar el efecto de loop sin fin en el autoscroll
            navContainer.innerHTML = buttonsHtml + buttonsHtml;

            // Iniciar scroll interactivo
            initCategoryScroll();
        }

        // --- SCROLL CATEGORÍAS (FLECHAS PC) ---
        ...
        // --- AUTO SCROLL CATEGORÍAS MEJORADO ---
        let isUserInteracting = false;
        function initCategoryScroll() {
            const nav = document.getElementById('category-nav');
            if(!nav) return;

            // Pausar movimiento automático cuando el usuario interactúa (toca o pone el cursor)
            ...
            // Espera un segundo tras soltar antes de volver a moverse automático
            ...
            setInterval(() => {
                if(!isUserInteracting) {
                    if(nav.scrollLeft >= nav.scrollWidth / 2) {
                        // Loop imperceptible hacia el inicio
                        nav.scrollLeft = 0;
                    } else {
        ...
        // --- FUNCION DEL BOTÓN NETFLIX RECOMENDADO ---
        ...
        // --- RENDER ALL CATEGORIES (TARJETAS E-COMMERCE PREMIUM) ---
        function renderAllCategories() {
            const container = document.getElementById('all-products-container');
            const order = ['pantallas', 'dobles', 'triples', 'mega', 'tv_deportes', 'musica', 'herramientas', 'redes'];
            let fullHtml = '';
            ...
            order.forEach((catId) => {
                ...
                sectionHtml += data.items.map((item, index) => {
                    ...
                    // TARJETAS SHOPIFY-STYLE: Info clara, precio destacado, botón full-width o prominente, Hover con colores de marca
                    return `
